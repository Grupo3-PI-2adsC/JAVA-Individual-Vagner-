...
//        System.out.println("aisdpasindpas");
        ...
        if (teste) {
            ...
            String tipoComponente = "Rede";
            Long limiteRede = con.queryForObject("""
                        SELECT metricaEstabelecida FROM tipoComponente 
                        WHERE nomeComponente = '%s';""".formatted(tipoComponente), Long.class);


            if (pacotesRecebidosRede > limiteRede){
                Slack.sendSlackAlert("Disco", fkMaquina, pacotesRecebidosRede);
            } else if (pacotesEnviadosRede > limiteRede) {
                Slack.sendSlackAlert("Disco", fkMaquina, pacotesEnviadosRede);
            }
