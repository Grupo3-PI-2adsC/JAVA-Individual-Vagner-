...
public class ProcessadorCp extends Componente {
    ...
    public void buscarInfosVariaveis(Boolean teste) {
        ...
        if (teste) {
            ...
            String tipoComponente = "Processador";
            Double limiteProcessador = con.queryForObject("""
                        SELECT metricaEstabelecida FROM tipoComponente 
                        WHERE nomeComponente = '%s';""".formatted(tipoComponente), Double.class);


            if (emUsoProcessador > limiteProcessador){
                Slack.sendSlackAlert("Disco", fkMaquina, emUsoProcessador);
            }
